...
def normalize_bin_label(label: str) -> str:
    """Return canonical bin label such as 3x3/t10.

    Older managers and manual outputs can encode the same bin in slightly
    different ways (path separators, filename-only labels, or index.json).  The
    Run UI should group by the canonical bin, not by a fragile raw path string.
    """
    s = str(label or "").replace("\\", "/").strip().strip("/")
    if not s:
        return ""
    # If a longer path was supplied, keep the part after 02_bin/ when possible.
    marker = "/02_bin/"
    low = s.lower()
    if marker in low:
        pos = low.rfind(marker)
        s = s[pos + len(marker):]
    # Common folder form: 3x3/t10, 3x3\\t10, 3x3_t10, images_3x3_t10.h5.
    m = re.search(r"(?i)(\d+)\s*x\s*(\d+)\s*[/_\\\- ]*t\s*(\d+)", s)
    if m:
        b1 = int(m.group(1))
        b2 = int(m.group(2))
        # DXB bins are square. If a malformed label has different b1/b2, keep b1
        # but do not fail scanning.
        return f"{b1}x{b1}/t{int(m.group(3))}"
    m = re.search(r"(?i)space[_\- ]*bin[=: ]+(\d+).*time[_\- ]*bin[=: ]+(\d+)", s)
    if m:
        b = int(m.group(1)); t = int(m.group(2))
        return f"{b}x{b}/t{t}"
    return s


def bin_label_from_index_or_path(p: Path, channel_root: Path, idx_info: dict[str, Any]) -> tuple[str, str]:
    """Return (canonical_label, raw_path_label)."""
    # Prefer index.json because it is the authoritative recipe metadata for DXB bins.
    try:
        sb = idx_info.get("space_bin", None)
        tb = idx_info.get("time_bin", None)
        if sb not in [None, ""] and tb not in [None, ""]:
            b = int(float(sb)); t = int(float(tb))
            return f"{b}x{b}/t{t}", f"index:{b}x{b}/t{t}"
    except Exception:
        pass

    raw_label = ""
    try:
        raw_label = "/".join(Path(p).parent.relative_to(Path(channel_root) / "02_bin").parts)
    except Exception:
        raw_label = Path(p).parent.name

    # Rescue from filename/path if relative_to failed or returned an unexpected label.
    canonical = normalize_bin_label(raw_label)
    if not re.match(r"^\d+x\d+/t\d+$", canonical):
        canonical_from_path = normalize_bin_label(str(p))
        if re.match(r"^\d+x\d+/t\d+$", canonical_from_path):
...
def scan_binned_h5(analysis_root: str, refresh_token: int = 0) -> list[dict[str, Any]]:
    ...
    if not root.exists():
        return []
    # Normal DXB Manager outputs: 02_bin/<space>x<space>/t<time>/images_...h5
    # Include nested files, but ignore safety/temp folders.  Use rglob rather
    # than relying on a single exact glob pattern so old/new outputs are both seen.
    files = sorted([p for p in root.rglob("*.h5") if "02_bin" in p.parts and p.name.lower().startswith("images")])
    out: list[dict[str, Any]] = []
    for p in files:
        if p.name.endswith(".tmp") or p.name.endswith(".part") or "_incomplete" in p.parts:
            continue
        dataset_id, data_label, channel_root = infer_dataset_label(root, p)
        idx_info = read_json(p.parent / "index.json", default={}) or {}
        bin_key, bin_path_label = bin_label_from_index_or_path(p, channel_root, idx_info)
        info = H5Info(
            path=str(p),
        ...
        try:
            with h5py.File(p, "r") as hf:
                ...
                # Access shape only; for ExternalLink this also verifies the link target.
            ...
            info.status = "error"
            msg = str(e)
            if str(idx_info.get("status", "")) == "done_alias":
                msg += " | This looks like a 1x1/t1 alias. If it was made by an older DXB H5 Manager, repair/recreate the alias because the ExternalLink may be relative to _incomplete."
            info.error = msg
        d = asdict(info)
        d["bin_key"] = bin_key
        d["bin_path_label"] = bin_path_label
        if idx_info:
            d["index_status"] = idx_info.get("status", "")
            d["index_source_h5"] = idx_info.get("source_h5", "")
            d["index_space_bin"] = idx_info.get("space_bin", "")
            d["index_time_bin"] = idx_info.get("time_bin", "")
        out.append(d)
    return out
...
def is_1x1_bin_label(bin_label: str) -> bool:
    ...
    return s.startswith("1x1/t1") or s == "1x1" or s.startswith("1/t1") or s in {"1x1", "1x1/t01", "1/t01"}
...
def make_bin_matrix(infos: list[dict[str, Any]], include_1x1: bool = False) -> pd.DataFrame:
    ...
    if not ready:
        return pd.DataFrame()
    bins = sorted({x.get("bin_key") or x.get("bin_label", "") for x in ready}, key=bin_sort_key)
    rows = []
    for (ds, lab), grp in sorted(pd.DataFrame(ready).groupby(["dataset_id", "data_label"]), key=lambda kv: (str(kv[0][0]), str(kv[0][1]))):
        ...
        have = set((grp["bin_key"] if "bin_key" in grp.columns else grp["bin_label"]).astype(str).tolist())
        for b in bins:
...
def filter_h5_infos(
    ...
    for x in infos:
        ...
        if labels and x.get("data_label") not in labels:
            continue
        if bins and "all" not in bins:
            wanted_bins = {normalize_bin_label(b) for b in bins}
            x_bin = normalize_bin_label(x.get("bin_key") or x.get("bin_label", ""))
            if x_bin not in wanted_bins:
                continue
...
def default_bin_filter(bin_labels: list[str]) -> list[str]:
    ...
    for preferred in ["3x3/t10", "10x10/t1", "8x8/t1", "5x5/t1", "3x3/t1"]:
...
def main():
    ...
    with tab_run:
        if not ready_infos:
            ...
            error_infos = [x for x in h5_infos if x.get("status") == "error"]
            if error_infos:
                st.error(f"H5は見つかりましたが、読み込み確認で {len(error_infos)} 件がerrorでした。Advanced > Detected でも確認できます。")
                cols = ["dataset_id", "data_label", "bin_label", "rel_path", "index_status", "error"]
                st.dataframe(pd.DataFrame(error_infos)[[c for c in cols if c in pd.DataFrame(error_infos).columns]], use_container_width=True, height=min(360, 80 + 32 * len(error_infos)))
            return

        # Lightweight project summary
        datasets_all = sorted({x.get("dataset_id", "") for x in ready_infos})
        bin_labels_all = sorted({x.get("bin_key") or x.get("bin_label", "") for x in ready_infos}, key=bin_sort_key)
        ready_count_by_bin = Counter(x.get("bin_key") or x.get("bin_label", "") for x in ready_infos)
        all_count_by_bin = Counter(x.get("bin_key") or x.get("bin_label", "") for x in h5_infos)
        error_count_by_bin = Counter((x.get("bin_key") or x.get("bin_label", "")) for x in h5_infos if x.get("status") == "error")
        has_non_1x1_bins = any(not is_1x1_bin_label(b) for b in bin_labels_all)
        c1, c2, c3, c4, c5 = st.columns(5)
        c1.metric("Datasets", len(datasets_all))
        c2.metric("Ready H5", len(ready_infos))
        c3.metric("Bin labels", len(bin_labels_all))
        c4.metric("H5 errors", len([x for x in h5_infos if x.get("status") == "error"]))
        c5.metric("Existing runs", len(run_rows))
        with st.expander("Bin scan counts / error check", expanded=False):
            count_rows = []
            for b in sorted(set(list(all_count_by_bin.keys()) + list(ready_count_by_bin.keys()) + list(error_count_by_bin.keys())), key=bin_sort_key):
                count_rows.append({
                    "bin": b,
                    "ready": int(ready_count_by_bin.get(b, 0)),
                    "error": int(error_count_by_bin.get(b, 0)),
                    "total_found": int(all_count_by_bin.get(b, 0)),
                })
            if count_rows:
                st.dataframe(pd.DataFrame(count_rows), use_container_width=True, hide_index=True)
            error_infos_all = [x for x in h5_infos if x.get("status") == "error"]
            if error_infos_all:
                st.warning("Some bin H5 files were found but could not be opened as /entry/data/images.")
                edf = pd.DataFrame(error_infos_all)
                cols = ["dataset_id", "data_label", "bin_label", "bin_path_label", "rel_path", "index_status", "error"]
                st.dataframe(edf[[c for c in cols if c in edf.columns]], use_container_width=True, height=min(360, 80 + 30 * len(edf)))
        ...
        with st.form("acf_simple_select_form"):
            ...
            with s2:
                ...
                default_bin = preferred[0] if preferred else (visible_bins_default[0] if visible_bins_default else (bin_labels_all[0] if bin_labels_all else "all"))
                bin_choices_base = visible_bins_default if visible_bins_default else bin_labels_all
                bin_choices = ["all"] + bin_choices_base
                ...
                bin_choice = st.selectbox(
                    ...
                    format_func=lambda b: "all bins" if b == "all" else f"{b} ({ready_count_by_bin.get(b, 0)} ready / {all_count_by_bin.get(b, 0)} found)",
                    key="acf_simple_bin",
                )
            with s3:
                hide_existing = st.checkbox("既存runなしだけ", value=False, key="acf_simple_missing_only")
                show_1x1 = st.checkbox("1x1/t1も使う", value=not has_non_1x1_bins, key="acf_simple_show_1x1")

            # Rebuild candidate list using the values currently in the form.
            datasets_filter = [] if dataset_choice == "all" else [dataset_choice]
            bins_filter = [] if (not bin_choice or bin_choice == "all") else [bin_choice]
            candidates = filter_h5_infos(
                h5_infos,
                datasets=datasets_filter,
                labels=None,
                bins=bins_filter,
                include_1x1=(show_1x1 or is_1x1_bin_label(bin_choice)),
                ready_only=True,
            ...
            if bin_choice != "all" and ready_count_by_bin.get(bin_choice, 0) != len(option_map) and not hide_existing and dataset_choice == "all":
                st.warning(
                    f"{bin_choice} は ready={ready_count_by_bin.get(bin_choice, 0)} / found={all_count_by_bin.get(bin_choice, 0)} 件です。"
                    " 期待より少ない場合は上の Bin scan counts / error check を開いて error 行を確認してください。"
                )
        ...
        if selected_infos:
            with st.expander("Selected details", expanded=False):
                ...
                view_cols = ["dataset_id", "data_label", "bin_label", "bin_key", "shape_text", "dtype", "has_mask", "existing_runs", "existing_packages", "rel_path"]
    ...
    with tab_advanced:
        ...
        with adv_tab1:
            ...
            if h5_infos:
                ...
                cols = ["dataset_id", "data_label", "bin_label", "bin_key", "bin_path_label", "shape_text", "dtype", "has_mask", "existing_runs", "existing_packages", "status", "index_status", "rel_path", "error"]
