...
起動
    streamlit run DXB_h5_manager.py
...
def save_json(path: Path, obj: Any):
    """Atomically write JSON to avoid partially-read progress/manifest files."""
    path = Path(path)
...
def discover_raw_binary_stack_folders(root: Path, width: int, height: int, raw_dtype: str,
                                      offset_bytes: int = 0, max_depth: int = 8,
                                      min_count: int = 1) -> List[Tuple[Path, List[Path]]]:
    """Return folders that directly contain headerless RAW frames.

    This is the safe batch mode for extensionless RAW: the scan is stat-only and
    requires exact byte-size matches. It intentionally skips analysis/output
    folders so an Analysis root can accidentally be chosen without re-importing
    generated H5/bin directories.
    """
    root = Path(root)
    if not root.exists() or not root.is_dir():
        return []
    skip_dir_names = {"01_h5", "02_bin", "_incomplete", ".dxb_jobs", "preview", "__pycache__"}
    out: List[Tuple[Path, List[Path]]] = []
    for cur, dirs, files in bounded_walk(root, max_depth=max_depth):
        ...
        if len(raw_paths) >= int(min_count):
            out.append((cur, sorted(raw_paths, key=natural_key)))
    return sorted(out, key=lambda x: natural_key(x[0]))


def guess_dataset_channel_for_raw_folder(raw_dir: Path, source_root_base: Optional[Path] = None) -> Tuple[str, str, str]:
    """Guess dataset/channel for a manually or batch-added RAW folder."""
    raw_dir = Path(raw_dir)
    base = Path(source_root_base) if source_root_base else None
    if base is None or not base.exists():
        base = raw_dir.parent if raw_dir.parent.exists() else raw_dir
    try:
        rel = raw_dir.resolve().relative_to(base.resolve())
        parts = rel.parts
    except Exception:
        parts = ()
    if len(parts) >= 2:
        dataset_dir = base / parts[0]
        dataset_id = safe_name(parts[0], "dataset")
        channel = infer_channel(dataset_dir, raw_dir)
        source_root_out = str(dataset_dir)
    elif len(parts) == 1:
        dataset_id = safe_name(parts[0] or raw_dir.name, "dataset")
        channel = infer_channel(base, raw_dir)
        source_root_out = str(base)
    else:
        dataset_id = safe_name(raw_dir.parent.name or raw_dir.name, "dataset")
        channel = infer_channel(raw_dir.parent if raw_dir.parent.exists() else raw_dir, raw_dir)
        source_root_out = str(raw_dir.parent if raw_dir.parent.exists() else raw_dir)
    return dataset_id, safe_name(channel, "default"), source_root_out


def make_raw_binary_import_row(raw_dir: Path, raw_paths: List[Path], raw_width: int, raw_height: int,
                               raw_dtype: str, raw_offset_bytes: int,
                               source_root_base: Optional[Path] = None, rank: int = 1,
                               run_default: bool = True) -> Dict[str, Any]:
    dataset_guess, channel_guess, source_root_out = guess_dataset_channel_for_raw_folder(raw_dir, source_root_base)
    return {
        "run": bool(run_default),
        ...
        "mask_path": candidate_mask_near(Path(source_root_out), Path(raw_dir), max_depth=3),
        "frames": int(len(raw_paths)),
        "height": int(raw_height),
        "width": int(raw_width),
        "dtype": str(raw_dtype),
        "notes": (
            "manual/batch RAW folder; "
            f"expected_file_size={raw_binary_expected_bytes(int(raw_width), int(raw_height), str(raw_dtype), int(raw_offset_bytes))} bytes; "
            f"offset={int(raw_offset_bytes)}"
        ),
        "source_root": source_root_out,
        "raw_frame_time_sec": None,
    }


def merge_scan_rows(old_df, new_rows: List[Dict[str, Any]]) -> pd.DataFrame:
    """Merge newly detected rows into the scan table without duplicating source folders."""
    new_df = pd.DataFrame(new_rows)
    if old_df is None or getattr(old_df, "empty", True):
        return new_df
    merged = pd.concat([old_df, new_df], ignore_index=True)
    subset = [c for c in ["source_type", "selected_sources", "dataset_id", "channel"] if c in merged.columns]
    if subset:
        merged = merged.drop_duplicates(subset=subset, keep="last")
    return merged.reset_index(drop=True)


def annotate_import_existing_status(df: pd.DataFrame, analysis_root: Optional[Path]) -> pd.DataFrame:
    """Add canonical_h5/existing_h5 columns for import candidates."""
    out = df.copy()
    if analysis_root is None or not Path(analysis_root).exists() or out.empty:
        out["canonical_h5"] = ""
        out["existing_h5"] = False
        return out
    paths = []
    exists = []
    for _, row in out.iterrows():
        h5 = canonical_h5_path(Path(analysis_root), safe_name(row.get("dataset_id", "dataset"), "dataset"), safe_name(row.get("channel", "default"), "default"))
        paths.append(str(h5))
        exists.append(bool(h5.exists()))
    out["canonical_h5"] = paths
    out["existing_h5"] = exists
    return out
...
def _parse_time_bin_from_path(path: Path, default: int = 1) -> int:
    """Extract tN from a path such as .../02_bin/8x8/t4/images_8x8_t4.h5."""
    try:
        for part in Path(path).parts[::-1]:
            m = re.fullmatch(r"t(\d+)", str(part).lower())
            if m:
...
def update_h5_time_metadata(
    h5_path: Path,
    raw_frame_time_sec: float,
    time_bin: Optional[int] = None,
    frame_time_source: str = "manual_time_editor",
    update_exposure_array: bool = False,
) -> Dict[str, Any]:
    """Update time metadata of an existing DXB H5 without rewriting image data.

    This edits only HDF5 attrs/meta and nearby JSON manifests/index files.
    For binned H5 files, pass the bin factor as time_bin; when omitted it is
    read from existing attrs/meta or inferred from the parent folder name tN.
    """
    h5_path = Path(h5_path)
    raw = _to_optional_positive_float(raw_frame_time_sec)
    if raw is None:
        raise ValueError(f"raw_frame_time_sec must be a positive number: {raw_frame_time_sec!r}")
    with h5py.File(h5_path, "r+") as hf:
        if "/entry/data/images" not in hf:
            raise KeyError(f"{h5_path}: /entry/data/images not found")
        dset = hf["/entry/data/images"]
        existing = get_h5_time_metadata(hf, fallback_texts=[str(h5_path)])
        tb = time_bin
        if tb is None:
            tb = int(existing.get("time_bin") or _parse_time_bin_from_path(h5_path, default=1) or 1)
        tb = max(1, int(tb))
        meta = apply_time_metadata_to_images_dataset(dset, raw, time_bin=tb, frame_time_source=frame_time_source)
        meta["time_updated_at"] = now_iso()
        g_meta = hf.require_group("entry").require_group("meta")
        write_time_metadata_to_meta_group(g_meta, meta)
        try:
            if "time_updated_at" in g_meta:
                del g_meta["time_updated_at"]
            g_meta.create_dataset("time_updated_at", data=meta["time_updated_at"], dtype=h5py.string_dtype("utf-8"))
        except Exception:
            pass
        if update_exposure_array and "/entry/data/exposure_time" in hf:
            try:
                exp = hf["/entry/data/exposure_time"]
                exp[...] = np.full(exp.shape, float(raw), dtype=exp.dtype)
                meta["exposure_time_array_updated"] = True
            except Exception as e:
                meta["exposure_time_array_updated"] = False
                meta["exposure_time_array_error"] = f"{type(e).__name__}: {e}"
    # Mirror to local manifest/index JSON when present.
    for jp in [h5_path.parent / IMAGES_MANIFEST, h5_path.parent / "index.json"]:
        if jp.exists():
            info = load_json(jp, default={})
            if isinstance(info, dict):
                info.update({
                    "raw_frame_time_sec": meta.get("raw_frame_time_sec"),
                    "time_bin": meta.get("time_bin"),
                    "effective_frame_time_sec": meta.get("effective_frame_time_sec"),
                    "frame_time_source": meta.get("frame_time_source"),
                    "time_updated_at": meta.get("time_updated_at"),
                })
                save_json(jp, info)
    return meta


def update_channel_time_metadata(
    channel_root: Path,
    raw_frame_time_sec: float,
    update_bins: bool = True,
    update_exposure_array: bool = False,
) -> List[Dict[str, Any]]:
    """Update 01_h5/images.h5 and optionally every existing 02_bin output for one channel."""
    channel_root = Path(channel_root)
    rows: List[Dict[str, Any]] = []
    canonical = channel_root / "01_h5" / "images.h5"
    if canonical.exists():
        meta = update_h5_time_metadata(
            canonical,
            raw_frame_time_sec,
            time_bin=1,
            frame_time_source="manual_time_editor_01_h5",
            update_exposure_array=update_exposure_array,
        )
        rows.append({"kind": "01_h5", "h5": str(canonical), **meta})
    if update_bins:
        for bh5 in sorted((channel_root / "02_bin").glob("*x*/t*/images_*.h5")):
            tb = _parse_time_bin_from_path(bh5, default=1)
            meta = update_h5_time_metadata(
                bh5,
                raw_frame_time_sec,
                time_bin=tb,
                frame_time_source="manual_time_editor_from_01_h5",
                update_exposure_array=False,
            )
            rows.append({"kind": "02_bin", "h5": str(bh5), **meta})
    return rows


def build_time_edit_table(analysis_root: Path) -> pd.DataFrame:
    """Build an editable table of channel-level frame-time metadata."""
    df = scan_analysis_root(Path(analysis_root))
    if df.empty:
        return df
    out = df.copy()
    out["run"] = False
    out["current_raw_frame_time_sec"] = out.get("raw_frame_time_sec", None)
    out["new_raw_frame_time_sec"] = out.get("raw_frame_time_sec", None)
    keep = [
        "run", "dataset_id", "channel", "current_raw_frame_time_sec", "new_raw_frame_time_sec",
        "effective_frame_time_sec", "frame_time_source", "frames", "n_bins", "channel_root", "images_h5",
    ]
...
@st.cache_data(show_spinner=False)
def cached_build_time_edit_table(analysis_root_str: str, refresh_token: int = 0) -> pd.DataFrame:
    # Time table scanning can be slow on large projects, so cache it and refresh only on demand.
    return build_time_edit_table(Path(clean_path_text(analysis_root_str)))
...
def _make_worker_progress_cb(job_dir: Path, item_label: str, item_index: int, item_total: int, progress_mode: str = "log_only"):
    """Create a worker-safe progress callback.

    progress_json mode writes every coarse batch/block to progress.json.
    log_only mode minimizes JSON writes and treats worker_events.log as the
    reliable progress source; this avoids Streamlit UI/progress-file races on
    slow disks or network drives.
    """
    mode = str(progress_mode or "log_only").lower().strip()
    last_written = {"current": None, "stage": None, "message": None, "bucket": None}
    critical_stages = {"start", "item_start", "scan", "commit", "preview", "index", "done", "error", "failed", "cancelled"}

    def _cb(current, total, stage="running", message="", **_kwargs):
        ...
        bucket = 0
        if tot > 0:
            bucket = int((cur / max(1, tot)) * 100) // 5  # 5% buckets
        duplicate = (last_written["current"], last_written["stage"], last_written["message"]) == (cur, stg, msg)
        if duplicate:
            return
        should_write_json = mode != "log_only" or stg in critical_stages or cur == 0 or (tot > 0 and cur >= tot) or bucket != last_written.get("bucket")
        last_written.update({"current": cur, "stage": stg, "message": msg, "bucket": bucket})
        if msg:
            _append_job_log(job_dir, f"{item_label} | {stg} | {msg} ({cur}/{tot})")
...
def _worker_run_import_job(job_json_path: str) -> int:
    ...
    try:
        ...
        for idx, row_dict in enumerate(rows, start=1):
            ...
            cb = _make_worker_progress_cb(job_dir, item_label, idx, len(rows), progress_mode=str(settings.get("worker_progress_mode", "log_only")))
...
def render_import_worker_panel(analysis_root: Optional[Path]):
    ...
    if logp.exists():
        with st.expander("Worker log", expanded=status in ["queued", "running", "error", "cancelled"]):
            st.code(logp.read_text(encoding="utf-8", errors="replace")[-8000:], language="text")
    stderrp = job_dir / "worker_stderr.log"
    if stderrp.exists() and stderrp.stat().st_size > 0:
        with st.expander("Worker stderr", expanded=status == "error"):
            st.code(stderrp.read_text(encoding="utf-8", errors="replace")[-8000:], language="text")
    stdoutp = job_dir / "worker_stdout.log"
    if stdoutp.exists() and stdoutp.stat().st_size > 0:
        with st.expander("Worker stdout", expanded=False):
            st.code(stdoutp.read_text(encoding="utf-8", errors="replace")[-8000:], language="text")


def _project_history_path() -> Path:
    """Small local project history file for one-click project selection."""
    try:
        return Path.home() / ".dxb_h5_manager_projects.json"
    except Exception:
        return Path(__file__).resolve().parent / ".dxb_h5_manager_projects.json"


def load_project_history() -> List[Dict[str, Any]]:
    data = load_json(_project_history_path(), default={"projects": []})
    ...
    for p in projects:
        if not isinstance(p, dict):
            continue
        src = clean_path_text(p.get("source_root", ""))
        ana = clean_path_text(p.get("analysis_root", ""))
        if not src and not ana:
            continue
        key = (src, ana)
        if key in seen:
            continue
        seen.add(key)
        name = str(p.get("name") or Path(ana or src).name or "project")
        out.append({
            "name": name,
            "source_root": src,
            "analysis_root": ana,
...
def save_project_history(projects: List[Dict[str, Any]]) -> None:
    save_json(_project_history_path(), {"projects": projects[:20], "updated_at": now_iso()})


def remember_project(source_root_txt: str, analysis_root_txt: str, name: str = "") -> None:
    src = clean_path_text(source_root_txt)
    ana = clean_path_text(analysis_root_txt)
    if not src and not ana:
        return
    name = safe_name(name or Path(ana or src).name or "project", "project")
    projects = load_project_history()
    key = (src, ana)
    projects = [p for p in projects if (clean_path_text(p.get("source_root", "")), clean_path_text(p.get("analysis_root", ""))) != key]
    projects.insert(0, {"name": name, "source_root": src, "analysis_root": ana, "updated_at": now_iso()})
    save_project_history(projects)


def _set_project_session(source_root_txt: str = "", analysis_root_txt: str = "") -> None:
    src = clean_path_text(source_root_txt)
    ana = clean_path_text(analysis_root_txt)
    if src:
        st.session_state["source_root_txt"] = src
        st.session_state["sidebar_source_root"] = src
    if ana:
        st.session_state["analysis_root_txt"] = ana
        st.session_state["sidebar_analysis_root"] = ana


def _short_path_label(path_txt: str, max_len: int = 52) -> str:
    txt = clean_path_text(path_txt)
    if len(txt) <= max_len:
        return txt
    return "…" + txt[-max_len:]


def list_child_dirs_for_picker(parent_txt: str, max_items: int = 40) -> List[Path]:
    parent = Path(clean_path_text(parent_txt))
...
def render_project_picker_sidebar() -> None:
    """Click-first project selector.

    Streamlit cannot reliably open the native OS folder chooser from a browser app,
    so this provides two stable click-based paths:
      1. recent saved projects
      2. one-level folder buttons under a parent folder
    """
    recent = load_project_history()
    if recent:
        st.caption("Recent projects")
        for i, p in enumerate(recent[:5]):
            label = f"📁 {p.get('name', 'project')}"
            help_txt = f"Source: {p.get('source_root','')}\nAnalysis: {p.get('analysis_root','')}"
            if st.button(label, key=f"project_recent_{i}", width="stretch", help=help_txt):
                _set_project_session(p.get("source_root", ""), p.get("analysis_root", ""))
    ...
    with st.expander("Click folder picker", expanded=False):
        saved_parent = clean_path_text(st.session_state.get("project_picker_parent", ""))
        current_analysis = clean_path_text(st.session_state.get("analysis_root_txt", ""))
        current_source = clean_path_text(st.session_state.get("source_root_txt", ""))
        if saved_parent:
            default_parent = saved_parent
        elif current_analysis:
            default_parent = str(Path(current_analysis).parent)
        elif current_source:
            default_parent = str(Path(current_source).parent)
        else:
            default_parent = ""
        parent_txt = st.text_input(
            "Parent folder",
            value=default_parent,
            placeholder=r"D:\DXB_projects or F:\beamtime_data",
            key="project_picker_parent_input",
        )
        st.session_state["project_picker_parent"] = parent_txt
        dirs = list_child_dirs_for_picker(parent_txt)
        if not dirs:
            st.caption("Parent folder配下のフォルダをここに表示します。最初だけ親フォルダパスを入れてください。")
        else:
            st.caption("クリックでSource/Analysisに設定できます。")
            for i, d in enumerate(dirs):
                c0, c1, c2 = st.columns([2.3, 1, 1])
                with c0:
                    st.caption(_short_path_label(str(d), 42))
                with c1:
                    if st.button("Source", key=f"pick_source_{i}", width="stretch"):
                        _set_project_session(source_root_txt=str(d))
                with c2:
                    if st.button("Analysis", key=f"pick_analysis_{i}", width="stretch"):
                        _set_project_session(analysis_root_txt=str(d))
...
def main():
    ...
    inject_css()
    st.title(APP_TITLE)
    st.caption("Raw → canonical 01_h5/images.h5 と、後からBin作成をまとめて扱う軽量Managerです。")

    with st.sidebar:
        st.header("Project")
        render_project_picker_sidebar()
        st.divider()
        ...
        with save_c1:
            if st.button("Save project", width="stretch", key="save_current_project_btn"):
                remember_project(source_root_txt, analysis_root_txt)
                st.success("Saved")
        with save_c2:
            if st.button("Refresh", width="stretch", key="project_refresh_btn"):
                st.session_state["analysis_scan_refresh_token"] = int(st.session_state.get("analysis_scan_refresh_token", 0)) + 1
                st.session_state["overview_refresh_token"] = int(st.session_state.get("overview_refresh_token", 0)) + 1
                st.rerun()
        st.divider()
        st.caption("ProjectはRecentボタンかFolder pickerでクリック選択できます。手入力もそのまま使えます。")
    ...
    perf = default_perf()
    for _tok in ["overview_refresh_token", "analysis_scan_refresh_token", "raw_scan_refresh_token", "catalog_refresh_token", "time_refresh_token", "time_editor_version"]:
        st.session_state.setdefault(_tok, 0)

    tab_overview, tab_import, tab_bin, tab_time, tab_catalog, tab_settings = st.tabs(["Overview", "Import H5", "Bin Manager", "Time", "Catalog", "Settings"])
    ...
    with tab_import:
        ...
        if add_manual_raw_btn:
            ...
            if not raw_dir.exists() or not raw_dir.is_dir():
                ...
                if not raw_paths:
                    ...
                    base_for_guess = source_root if source_root and source_root.exists() else raw_dir.parent
                    row = make_raw_binary_import_row(
                        raw_dir, raw_paths,
                        int(raw_binary_width), int(raw_binary_height), str(raw_binary_dtype), int(raw_binary_offset_bytes),
                        source_root_base=base_for_guess,
                    )
                    st.session_state["scan_df"] = merge_scan_rows(st.session_state.get("scan_df"), [row])
                    st.success(f"RAW folder added: {len(raw_paths)} frames")

        with st.expander("Batch add RAW folders under a parent", expanded=False):
            batch_c1, batch_c2, batch_c3 = st.columns([3, 1, 1])
            with batch_c1:
                batch_raw_root_txt = st.text_input(
                    "RAW parent/root",
                    value=st.session_state.get("batch_raw_root_txt", clean_path_text(source_root_txt)),
                    placeholder=r"F:\...\parent_of_raw_folders",
                    key="batch_raw_root_input",
                )
                st.session_state["batch_raw_root_txt"] = batch_raw_root_txt
            with batch_c2:
                batch_raw_depth = st.number_input("RAW scan depth", min_value=0, max_value=12, value=6, step=1, key="batch_raw_depth")
            with batch_c3:
                add_batch_raw_btn = st.button("Add RAW folders", type="secondary", width="stretch", key="add_batch_raw_folders_btn")
            st.caption("親フォルダ以下から、指定 width/height/dtype/offset と完全一致する拡張子なしRAWを含むフォルダを一括追加します。既存H5は下の候補表で existing_h5=True と表示されます。")
            if add_batch_raw_btn:
                batch_root = Path(clean_path_text(batch_raw_root_txt))
                if not batch_root.exists() or not batch_root.is_dir():
                    st.error("RAW parent/root が見つかりません。")
                else:
                    found = discover_raw_binary_stack_folders(
                        batch_root,
                        int(raw_binary_width), int(raw_binary_height), str(raw_binary_dtype), int(raw_binary_offset_bytes),
                        max_depth=int(batch_raw_depth), min_count=1,
                    )
                    if not found:
                        expected = raw_binary_expected_bytes(int(raw_binary_width), int(raw_binary_height), str(raw_binary_dtype), int(raw_binary_offset_bytes))
                        st.warning(f"RAWフォルダが見つかりませんでした。expected file size = {expected} bytes")
                    else:
                        rows = [
                            make_raw_binary_import_row(
                                raw_dir, raw_paths,
                                int(raw_binary_width), int(raw_binary_height), str(raw_binary_dtype), int(raw_binary_offset_bytes),
                                source_root_base=batch_root, rank=i + 1,
                            )
                            for i, (raw_dir, raw_paths) in enumerate(found)
                        ]
                        st.session_state["scan_df"] = merge_scan_rows(st.session_state.get("scan_df"), rows)
                        st.success(f"Added {len(rows)} RAW folder(s).")
        ...
        if df0 is not None and not df0.empty:
            ...
            if "run" not in df0.columns:
                df0["run"] = True
            if analysis_root:
                df0 = annotate_import_existing_status(df0, analysis_root)
            st.markdown("### Candidates")
            ctl1, ctl2, ctl3, ctl4 = st.columns([1, 1, 1.4, 2.2])
            with ctl1:
                if st.button("Check all", key="import_candidates_check_all", width="stretch"):
                    df0["run"] = True
                    st.session_state["scan_df"] = df0
                    st.rerun()
            with ctl2:
                if st.button("Uncheck all", key="import_candidates_uncheck_all", width="stretch"):
                    df0["run"] = False
                    st.session_state["scan_df"] = df0
                    st.rerun()
            with ctl3:
                if st.button("Check missing H5 only", key="import_candidates_missing_only", width="stretch"):
                    if "existing_h5" in df0.columns:
                        df0["run"] = ~df0["existing_h5"].astype(bool)
                    st.session_state["scan_df"] = df0
                    st.rerun()
            with ctl4:
                if "existing_h5" in df0.columns:
                    st.caption(f"existing_h5=True は既に 01_h5/images.h5 がある候補です。missing={int((~df0['existing_h5'].astype(bool)).sum())}, existing={int(df0['existing_h5'].astype(bool).sum())}")
            ...
            edited = st.data_editor(
                ...
                disabled=["candidate_rank", "score", "source_type", "frames", "height", "width", "dtype", "source_root", "existing_h5", "canonical_h5"],
                key="scan_editor_v4_batch_skip",
            )
            ...
            wopt1, wopt2, wopt3 = st.columns([1.3, 1, 2.7])
            with wopt1:
                skip_existing_before_worker = st.checkbox("Skip existing before worker", value=True, key="import_skip_existing_before_worker")
            with wopt2:
                worker_progress_mode = st.selectbox("Worker progress", ["log_only", "progress_json"], index=0, key="import_worker_progress_mode")
            with wopt3:
                st.caption("log_only は progress.json 更新を減らし、worker_events.log を主な進捗源にします。UI progress が不安定なPCではこちら推奨です。")
            ...
            if run_import:
                if not analysis_root:
                    ...
                    if selected.empty:
                        ...
                        if skip_existing_before_worker and str(import_mode) == "Build if missing":
                            selected = annotate_import_existing_status(selected, analysis_root)
                            skipped_existing = selected[selected.get("existing_h5", False) == True].copy()  # noqa: E712
                            selected = selected[selected.get("existing_h5", False) != True].copy()  # noqa: E712
                            if not skipped_existing.empty:
                                st.info(f"Skipped existing images.h5 before worker: {len(skipped_existing)} item(s).")
                        if selected.empty:
                            st.success("All selected items already have images.h5. Worker was not started.")
                            rebuild_project_catalog(analysis_root)
                            bump_project_refresh_tokens()
                        else:
                            # Launch heavy import in a separate worker process. Streamlit monitors progress.json/log files only.
                            rows = [
                            ...
                            "raw_binary_height": int(raw_binary_height),
                                ...
                                "worker_progress_mode": str(worker_progress_mode),
                            }
                            try:
    ...
    with tab_bin:
        ...
        if not analysis_root:
            ...
            if df.empty:
                ...
                hsel1, hsel2, hsel3 = st.columns([1, 1, 3])
                with hsel1:
                    if st.button("Check all ready", key="h5_select_all_ready", width="stretch"):
                        df_sel["run"] = df_sel["status"].eq("ready")
                with hsel2:
                    if st.button("Uncheck all", key="h5_select_none", width="stretch"):
                        df_sel["run"] = False
                with hsel3:
                    st.caption("H5 channel表のrun列を一括操作します。")
        ...
        if q is None or q.empty:
            ...
            q = q.copy()
            qctl1, qctl2, qctl3, qctl4 = st.columns([1, 1, 1.3, 2.7])
            with qctl1:
                if st.button("Check all queue", key="queue_check_all", width="stretch"):
                    q["run"] = True
                    st.session_state["bin_queue"] = q
            with qctl2:
                if st.button("Uncheck all queue", key="queue_uncheck_all", width="stretch"):
                    q["run"] = False
                    st.session_state["bin_queue"] = q
            with qctl3:
                if st.button("Check missing only", key="queue_check_missing", width="stretch"):
                    if "current_status" in q.columns:
                        q["run"] = q["current_status"].astype(str).ne("exists")
                    st.session_state["bin_queue"] = q
            with qctl4:
                if "current_status" in q.columns:
                    st.caption(f"missing={int(q['current_status'].astype(str).ne('exists').sum())}, exists={int(q['current_status'].astype(str).eq('exists').sum())}")
    ...
    with tab_time:
        st.subheader("Time metadata editor")
        st.markdown("""
        <div class="dxb-card">
        測定ごとに露光時間 / frame interval が違う場合、Import後でもここで <code>01_h5/images.h5</code> の時間メタデータを修正できます。<br>
        既存のBinにも反映すると、各Binの <code>effective_frame_time_sec = raw_frame_time_sec × time_bin</code> も更新されます。画像データ自体は書き換えません。
        </div>
        """, unsafe_allow_html=True)
        if not analysis_root:
            st.info("Analysis root を指定してください。")
        else:
            # Large projects are slow to scan.  Do not rescan on every Streamlit rerun;
            # keep the editable table in session_state and refresh only when requested.
            top_a, top_b, top_c = st.columns([1, 1, 3])
            with top_a:
                if st.button("Refresh time table", key="time_refresh_table", width="stretch"):
                    st.session_state["time_refresh_token"] = int(st.session_state.get("time_refresh_token", 0)) + 1
                    st.session_state.pop("time_edit_df", None)
                    st.session_state["time_editor_version"] = int(st.session_state.get("time_editor_version", 0)) + 1
            with top_b:
                if st.button("Keep table", key="time_keep_table", width="stretch"):
                    # No-op button for a clear UI affordance: this tab now keeps edits instead of reloading every rerun.
                    pass
            with top_c:
                st.caption("反映が遅く見えないよう、表はキャッシュします。実ファイルを再読込したい時だけ Refresh time table を押してください。")

            time_df = cached_build_time_edit_table(str(analysis_root), st.session_state.get("time_refresh_token", 0))
            if time_df.empty:
                st.info("canonical 01_h5/images.h5 がありません。先にImport H5を実行してください。")
            else:
                project_key = str(Path(analysis_root).resolve())

                def _bump_time_editor():
                    st.session_state["time_editor_version"] = int(st.session_state.get("time_editor_version", 0)) + 1

                def _store_time_df(df_new: pd.DataFrame, bump: bool = True):
                    st.session_state["time_edit_df"] = df_new.copy()
                    st.session_state["time_edit_project"] = project_key
                    if bump:
                        _bump_time_editor()

                def _current_edit_base() -> pd.DataFrame:
                    base = st.session_state.get("time_edit_df")
                    same_project = st.session_state.get("time_edit_project") == project_key
                    if base is None or getattr(base, "empty", True) or not same_project:
                        base = time_df.copy()
                        _store_time_df(base, bump=False)
                        return base
                    # Keep user-edited run/new values, but refresh read-only columns only after explicit table refresh.
                    try:
                        base = base.copy()
                        for col in time_df.columns:
                            if col not in base.columns:
                                base[col] = time_df[col]
                        return base[time_df.columns]
                    except Exception:
                        base = time_df.copy()
                        _store_time_df(base, bump=False)
                        return base

                edit_base = _current_edit_base()

                st.caption("runにチェックを入れ、new_raw_frame_time_sec [s] を測定ごとに編集してApplyしてください。例: 250 ms = 0.25, 500 ms = 0.50")
                tc1, tc2, tc3, tc4 = st.columns([1, 1, 1, 2])
                with tc1:
                    if st.button("Check all", key="time_check_all", width="stretch"):
                        edit_base = _current_edit_base().copy()
                        edit_base["run"] = True
                        _store_time_df(edit_base)
                with tc2:
                    if st.button("Uncheck all", key="time_uncheck_all", width="stretch"):
                        edit_base = _current_edit_base().copy()
                        edit_base["run"] = False
                        _store_time_df(edit_base)
                with tc3:
                    if st.button("Check missing", key="time_check_missing", width="stretch"):
                        edit_base = _current_edit_base().copy()
                        edit_base["run"] = edit_base["current_raw_frame_time_sec"].isna()
                        _store_time_df(edit_base)
                with tc4:
                    st.caption("チェックや一括入力は即時に表へ反映します。反映が消える場合に備えて、表の内部keyを毎回更新します。")

                # Re-read after possible check button edits.
                edit_base = _current_edit_base()

                st.markdown("#### Bulk set")
                bf1, bf2, bf3, bf4, bf5 = st.columns([1.1, 1, 1, 1, 1.2])
                with bf1:
                    bulk_raw_frame_time_sec = st.number_input(
                        "Bulk frame interval [s]",
                        min_value=0.0,
                        value=float(st.session_state.get("time_bulk_frame_sec", 0.05)),
                        step=0.001,
                        format="%.6f",
                        key="time_bulk_frame_sec",
                        help="例: 250 ms = 0.25, 500 ms = 0.50, 50 ms = 0.05",
                    )
                with bf2:
                    if st.button("Set checked", key="time_fill_checked", width="stretch"):
                        edit_base = _current_edit_base().copy()
                        edit_base.loc[edit_base["run"].astype(bool), "new_raw_frame_time_sec"] = float(bulk_raw_frame_time_sec)
                        _store_time_df(edit_base)
                with bf3:
                    if st.button("Set missing", key="time_fill_missing", width="stretch"):
                        edit_base = _current_edit_base().copy()
                        missing = edit_base["current_raw_frame_time_sec"].isna()
                        edit_base.loc[missing, "new_raw_frame_time_sec"] = float(bulk_raw_frame_time_sec)
                        edit_base.loc[missing, "run"] = True
                        _store_time_df(edit_base)
                with bf4:
                    if st.button("Set all", key="time_fill_all", width="stretch"):
                        edit_base = _current_edit_base().copy()
                        edit_base["new_raw_frame_time_sec"] = float(bulk_raw_frame_time_sec)
                        edit_base["run"] = True
                        _store_time_df(edit_base)
                with bf5:
                    if st.button("Copy current → new", key="time_copy_current", width="stretch"):
                        edit_base = _current_edit_base().copy()
                        edit_base["new_raw_frame_time_sec"] = edit_base["current_raw_frame_time_sec"]
                        _store_time_df(edit_base)

                # Re-read after possible bulk edits. Dynamic key prevents Streamlit's data_editor
                # from showing the previous widget state for one rerun.
                edit_base = _current_edit_base()
                editor_version = int(st.session_state.get("time_editor_version", 0))
                edited_time = st.data_editor(
                    ...
                    column_config={
                        "run": st.column_config.CheckboxColumn("run"),
                        "new_raw_frame_time_sec": st.column_config.NumberColumn("new_raw_frame_time_sec [s]", min_value=0.0, step=0.001, format="%.6f"),
                        "current_raw_frame_time_sec": st.column_config.NumberColumn("current_raw_frame_time_sec [s]", format="%.6f"),
                        "channel_root": st.column_config.TextColumn("channel_root", width="large"),
                        "images_h5": st.column_config.TextColumn("images_h5", width="large"),
                    },
                    disabled=[c for c in edit_base.columns if c not in ["run", "new_raw_frame_time_sec"]],
                    key=f"time_metadata_editor_v{editor_version}",
                )
                _store_time_df(edited_time, bump=False)

                opt1, opt2, opt3, opt4 = st.columns([1, 1, 1, 1.8])
                with opt1:
                    update_bins_too = st.checkbox(
                        "Also update existing bins",
                        value=False,
                        key="time_update_bins",
                        help="既存の02_binが多いと時間がかかります。これからBinを作る場合はOFFで十分です。",
                    )
                with opt2:
                    update_exposure_array = st.checkbox("Fill 01_h5 exposure_time array", value=False, key="time_update_exposure_array")
                with opt3:
                    refresh_after_apply = st.checkbox("Refresh scan after apply", value=False, key="time_refresh_after_apply")
                with opt4:
                    st.caption("速度優先なら bins/refresh はOFF推奨。H5属性更新自体は画像データを書き換えないので軽いです。")

                if st.button("Apply selected time metadata", type="primary", width="stretch", key="time_apply_selected"):
                    selected = edited_time[edited_time["run"] == True].copy()  # noqa: E712
                    if selected.empty:
                        st.warning("run=True の行がありません。")
                    else:
                        results = []
                        skipped = []
                        errors = []
                        apply_prog = st.progress(0.0)
                        apply_status = st.empty()
                        total_sel = len(selected)
                        for ii, (_, r) in enumerate(selected.iterrows(), start=1):
                            raw = _to_optional_positive_float(r.get("new_raw_frame_time_sec"))
                            label = f"{r.get('dataset_id')} / {r.get('channel')}"
                            apply_status.caption(f"Updating {ii}/{total_sel}: {label}")
                            if raw is None:
                                skipped.append({"target": label, "reason": "new_raw_frame_time_sec is blank; skipped"})
                                apply_prog.progress(ii / max(1, total_sel))
                                continue
                            try:
                                rows = update_channel_time_metadata(
                                    Path(str(r["channel_root"])),
                                    raw,
                                    update_bins=bool(update_bins_too),
                                    update_exposure_array=bool(update_exposure_array),
                                )
                                for rr in rows:
                                    results.append({"target": label, **rr})
                                # Update the visible table immediately instead of waiting for a rescan.
                                mask = (
                                    (st.session_state["time_edit_df"]["dataset_id"].astype(str) == str(r.get("dataset_id"))) &
                                    (st.session_state["time_edit_df"]["channel"].astype(str) == str(r.get("channel")))
                                )
                                st.session_state["time_edit_df"].loc[mask, "current_raw_frame_time_sec"] = float(raw)
                                st.session_state["time_edit_df"].loc[mask, "new_raw_frame_time_sec"] = float(raw)
                                st.session_state["time_edit_df"].loc[mask, "effective_frame_time_sec"] = float(raw)
                                st.session_state["time_edit_df"].loc[mask, "frame_time_source"] = "manual_time_editor_01_h5"
                                st.session_state["time_edit_df"].loc[mask, "run"] = False
                            except Exception as e:
                                errors.append({"target": label, "error": f"{type(e).__name__}: {e}"})
                            apply_prog.progress(ii / max(1, total_sel))

                        apply_status.caption("Apply finished")
                        if results:
                            st.success(f"Updated {len(results)} H5 file(s).")
                            st.dataframe(pd.DataFrame(results), width="stretch", height=min(360, 38 * (len(results) + 1)))
                            _bump_time_editor()
                            if refresh_after_apply:
                                st.session_state["time_refresh_token"] = int(st.session_state.get("time_refresh_token", 0)) + 1
                                st.session_state["analysis_scan_refresh_token"] = int(st.session_state.get("analysis_scan_refresh_token", 0)) + 1
                                st.session_state["catalog_refresh_token"] = int(st.session_state.get("catalog_refresh_token", 0)) + 1
                                try:
                                    rebuild_project_catalog(analysis_root)
                                except Exception as e:
                                    st.warning(f"Catalog refresh skipped: {type(e).__name__}: {e}")
                            else:
                                st.caption("表示は即時更新しました。Overview/Catalogへ完全反映したい場合は Refresh scan after apply をONにするか、後でCatalogをRebuildしてください。")
                        if skipped:
                            st.warning(f"Skipped {len(skipped)} selected row(s) because the new time was blank. Use Bulk set → Set checked / Set missing first.")
                            st.dataframe(pd.DataFrame(skipped), width="stretch", height=min(200, 38 * (len(skipped) + 1)))
                        if errors:
                            st.error(f"Failed {len(errors)} item(s).")
                            st.dataframe(pd.DataFrame(errors), width="stretch", height=min(240, 38 * (len(errors) + 1)))
